...
export const RecipeCard = ({recipe}: RecipeCardProps) => {
    ...
    return (
        <ThemeButton variant="label" onPress={handlePress}>
            <View style={tw`w-80 md:w-180 bg-[${colorTheme.primary}] rounded-xl shadow-lg p-6`}>
                ...
                <View style={tw`mt-4`}>
                    ...
                    {recipe.ingredients.map((ingredient) => (
                        <ThemeText style={tw`flex gap-1`} variant="caption" key={ingredient}>• {ingredient}</ThemeText>
                    ))}
                </View>
            </View>
